// This is What my HEader looks like before the Search functionality
...
// Header after Search Functionality been worked on
import React, { useState, useEffect } from "react";
import styles from "./navBar.module.css";
import { Link } from "react-router-dom";
import Categories from "../categories/";
...
function NavBar({ userPrompt, setUserPrompt, handleFilter }) {
  const [isMobile, setIsMobile] = useState(window.innerWidth <= 768);
  const [showNav, setShowNav] = useState(false);
  useEffect(() => {
    const handleScroll = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.pageYOffset > 5 && showNav) setShowNav(false);
    };
    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, [showNav]);
      ...
      <div className={styles.second}>
        ...
        <div className={styles.label_wrap}>
          ...
          <svg
            ...
            onClick={handleFilter}
          >
            <path d="m21 21-5.197-5.197M16 10.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0Z" />
          </svg>
        </div>
